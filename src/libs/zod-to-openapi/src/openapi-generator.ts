...
export class OpenAPIGenerator {
  ...
  private generateInlineParameters(
    ...
    if (isZodType(zodSchema, 'ZodObject')) {
      ...
      for (let [key, schema] of entries) {
        ...
        if (referencedSchema) {
          parameters.push(referencedSchema);
        }
        ...
        if (
          ...
          parameters.push(this.generateParameter(
  ...
  private getParameters(
    ...
    const paramQueryParameters = this.enhanceMissingParametersError(
      () => (params ? this.generateInlineParameters(params, 'query') : []),
      { location: 'query' }
    );

    const paramPathParameters = this.enhanceMissingParametersError(
        () => (params ? this.generateInlineParameters(params, 'path') : []),
        { location: 'path' }
    );

    const paramHeaderParameters = this.enhanceMissingParametersError(
        () => (params ? this.generateInlineParameters(params, 'header') : []),
        { location: 'header' }
    );
    ...
    return [...paramPathParameters, ...paramQueryParameters, ...paramHeaderParameters, ...queryParameters, ...headerParameters];
  }
